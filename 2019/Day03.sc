...
type PixelPath = Array[(Int, Int)]

object Vector {
  val vecPat = """([UDRL])(\d+)""".r
  def unapply(v: String): Option[(Char, Int)] = try {
    val vecPat(dir,distStr) = v
    Option((dir.head, distStr.toInt))
  } catch {
    case _: NumberFormatException => None
  }
}

val Vector(dir,dist) = "L72"

def toPixelPath(vectorPath: Array[String]): (Int, Int, PixelPath) = vectorPath.foldLeft((0,0, Array.empty[(Int, Int)])){
  case ((x, y, path), Vector('R', d)) => (x+d, y  , path ++ (1 to d).map(x+_).map(x=>(x,y)))
  case ((x, y, path), Vector('U', d)) => (x  , y+d, path ++ (1 to d).map(y+_).map(y=>(x,y)))
  case ((x, y, path), Vector('L', d)) => (x-d, y  , path ++ (1 to d).map(x-_).map(x=>(x,y)))
  case ((x, y, path), Vector('D', d)) => (x  , y-d, path ++ (1 to d).map(y-_).map(y=>(x,y)))
}

implicit class ManhattanDistOps(val self: (Int, Int)) extends AnyVal {
  def manDist: Int = Math.abs(self._1) + Math.abs(self._2)
}

for {
  in <- ins
  Array((_,_,f),(_,_,s)) = in.map(toPixelPath)
  intersections = f.intersect(s)
  nearest = intersections.minBy(_.manDist)
} {
  println(nearest)
  println(nearest.manDist)
}
...
for {
  ...
  Array((_,_,f),(_,_,s)) = in.map(toPixelPath)
  intersections = f.intersect(s)
  firstIntersection = intersections.minBy(p => f.indexOf(p) + s.indexOf(p) + 2)
} {
  println(firstIntersection)
  println(f.indexOf(firstIntersection))
  println(s.indexOf(firstIntersection))
  println(f.indexOf(firstIntersection) + s.indexOf(firstIntersection) + 2)
}
